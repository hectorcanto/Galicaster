...
#------------------------- POPUP ACTIONS ------------------------
    ...
    def on_cut(self,button): # TODO move to a proper place
        """Makes a cut on the current recording"""
        self.dispatcher.emit("cut-record")
        return True 

    def on_stream(self,button): # TODO move to a proper place
        """Starts or stops the streaming service"""
        if self.streaming:
            self.dispatcher.emit('start-streaming')
            self.streaming = True
        else:
            self.dispatcher.emit('stop-streaming')
            self.streaming = False
